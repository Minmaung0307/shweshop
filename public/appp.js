...
async function listFeedback(){
  ...
  let isAdmin = false;
  try { const t = await u.getIdTokenResult(true); isAdmin = t.claims?.admin === true; } catch {}
  if (!isAdmin) return; // non-admin: silently skip
  try {
    const snaps = await getDocs(collection(db,'feedback'));
    const wrap = document.getElementById('feedbackList'); if(!wrap) return;
    wrap.innerHTML = snaps.docs.map(d=>{
      const f = d.data();
      const when = (f.createdAt?.toDate?.() ?? new Date()).toLocaleString();
      return `<div class="card" style="margin:6px 0"><b>${f.name||'Anon'}</b> <span class="muted">${when}</span><div>${f.message||''}</div></div>`;
    }).join('');
  } catch(e) { console.warn('Feedback read skipped:', e.message||e); }
}
